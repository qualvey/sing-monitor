<template>
  <div v-if="!authed" class="min-h-screen flex items-center justify-center">
    <div class="w-full max-w-md bg-[#0b0f19] border border-slate-800 rounded-2xl p-8 shadow-2xl">
      <div class="text-center mb-8">
        ...
        <p class="text-sm text-slate-400 mt-1">请输入管理员密码</p>
      </div>
      <form @submit.prevent="doLogin" class="space-y-4">
        <input v-model="pwd" type="password" required placeholder="••••••••"
          class="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
  ...
  <div v-else class="min-h-screen">
    <!-- 顶部导航 -->
    <header class="sticky top-0 z-30 bg-[#131b2e]/80 backdrop-blur-md border-b border-slate-800/80">
        ...
        <div class="flex items-center justify-between h-16">
          ...
          <nav class="hidden md:flex items-center space-x-1 bg-slate-900/60 p-1 rounded-xl border border-slate-800">
            <button v-for="t in tabs" :key="t.id" @click="tab = t.id"
              :class="[tab === t.id ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200', 'px-4 py-2 text-sm font-medium rounded-lg transition-all']">
              {{ t.name }}
            </button>
          </nav>
          <div class="flex items-center space-x-2">
            <button @click="doImport" :disabled="loading" title="从 /etc/sing-box/config.json 同步导入"
              class="hidden sm:inline-flex px-3 py-1.5 text-xs text-slate-300 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg disabled:opacity-50">
              同步母配置
            </button>
