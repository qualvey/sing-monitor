<template>
  <div class="space-y-6">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <!-- 实时速率图表 -->
      <div class="lg:col-span-2 bg-white dark:bg-[#131b2e] border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
        <div class="flex items-center justify-between mb-2">
          <h3 class="text-base font-bold">⚡ 实时速率 (bytes/s) — {{ chartTargetName }}</h3>
          <select v-model="chartTarget" @change="chartSeries = []"
            class="px-2 py-1 text-xs bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:outline-none focus:border-indigo-500 max-w-[180px]">
      ...
      <div class="bg-white dark:bg-[#131b2e] border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-3">
        ...
        <div>
          <div class="text-xs text-slate-600 dark:text-slate-400 uppercase">上行速率</div>
          <div class="text-2xl font-bold text-indigo-300">{{ fmtBytes(globalUp) }}/s</div>
        </div>
        <div>
          <div class="text-xs text-slate-600 dark:text-slate-400 uppercase">下行速率</div>
          <div class="text-2xl font-bold text-cyan-300">{{ fmtBytes(globalDown) }}/s</div>
        </div>
        <div>
          <div class="text-xs text-slate-600 dark:text-slate-400 uppercase">连接状态</div>
      ...
      <table class="w-full text-left text-xs">
        ...
        <tbody class="divide-y divide-slate-800/60">
          <tr v-for="u in sortedUsers" :key="u.name" @click="chartTarget = u.name; chartSeries.length = 0"
            :class="['hover:bg-slate-100 dark:hover:bg-slate-200 dark:hover:bg-slate-200 dark:bg-slate-800/40 cursor-pointer transition-colors', chartTarget === u.name ? 'bg-indigo-500/5' : '']">
            ...
            <td class="py-3 px-4">
              <span :class="u.online ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-700',
