<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="bg-[#131b2e] border border-slate-800 rounded-2xl p-5">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold text-slate-400 uppercase">系统上行流量</span>
      ...
      <div class="bg-[#131b2e] border border-slate-800 rounded-2xl p-5">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold text-slate-400 uppercase">系统下行流量</span>
      ...
      <div class="bg-[#131b2e] border border-slate-800 rounded-2xl p-5">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold text-slate-400 uppercase">代理用户数</span>
          <span class="p-2.5 bg-emerald-500/10 rounded-xl text-emerald-400">👤</span>
        </div>
        <div class="mt-3 text-2xl font-bold">{{ users.length }} <span class="text-xs text-slate-400">({{ activeCount }} 活跃)</span></div>
      </div>
      <div class="bg-[#131b2e] border border-slate-800 rounded-2xl p-5">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold text-slate-400 uppercase">入站节点数</span>
          <span class="p-2.5 bg-amber-500/10 rounded-xl text-amber-400">🖧</span>
        </div>
        <div class="mt-3 text-2xl font-bold">{{ inbounds.length }} <span class="text-xs text-slate-400">节点服务</span></div>
      </div>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <!-- 周期流量 Top 榜（核心新功能） -->
      <div class="lg:col-span-2 bg-[#131b2e] border border-slate-800 rounded-2xl p-6">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="text-base font-bold">🔥 周期流量 Top 榜</h3>
            <p class="text-xs text-slate-400 mt-0.5">按当前计费周期内已用流量降序（周期 = 起始时间 + {{ defaultCycleDays }} 天）</p>
          </div>
        ...
        <div class="space-y-3">
          <div v-for="(u, i) in topPeriodUsers" :key="u.id"
            class="p-3.5 bg-slate-900/60 border border-slate-800 rounded-xl flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                ...
                <div class="text-xs text-slate-400 mt-0.5">周期：{{ fmtTime(u.cycle_start) }} ~ {{ fmtTime(u.cycle_end) }}</div>
              </div>
            </div>
            <div class="text-right">
              <div class="text-sm font-bold font-mono">{{ fmtBytes(u.period_total_bytes) }}</div>
              <div class="text-[11px] text-slate-400">↑{{ fmtBytes(u.period_up_bytes) }} ↓{{ fmtBytes(u.period_down_bytes) }}</div>
            </div>
          </div>
      ...
      <!-- 入站节点状态 -->
      <div class="bg-[#131b2e] border border-slate-800 rounded-2xl p-6">
        <div class="flex items-center justify-between mb-4">
        ...
        <div class="space-y-3">
          <div v-for="n in inbounds" :key="n.id"
            class="p-3 bg-slate-900/60 border border-slate-800 rounded-xl flex items-center justify-between">
            <div>
              <div class="text-sm font-semibold flex items-center gap-2">
                {{ n.tag }}
              ...
              <div class="text-xs text-slate-400 mt-0.5">端口 :{{ n.listen_port }} | {{ n.server_name || '内置路由' }}</div>
            </div>
...
function rankCls(i) {
  ...
  if (i === 1) return 'bg-slate-400/20 text-slate-300 border-slate-400/30'
