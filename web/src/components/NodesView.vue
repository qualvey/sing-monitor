<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between bg-white dark:bg-[#131b2e] border border-slate-200 dark:border-slate-800 p-4 rounded-2xl">
      <div>
        <h2 class="text-base font-bold">🖧 入站节点管理</h2>
        <p class="text-xs text-slate-600 dark:text-slate-400 mt-0.5">支持 VLESS + Reality 与 TUIC v5 入站节点管理</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="n in inbounds" :key="n.id"
        class="bg-white dark:bg-[#131b2e] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xl">
        <div class="flex items-start justify-between">
          <div class="flex items-center space-x-3">
            ...
            <div>
              ...
              <p class="text-xs text-slate-600 dark:text-slate-400 mt-0.5">监听: {{ n.listen || '::' }} : {{ n.listen_port }}</p>
            </div>
          </div>
          <div class="flex items-center space-x-2">
            <button @click="openEdit(n)" class="p-1.5 text-slate-600 dark:text-slate-400 hover:text-cyan-400 hover:bg-slate-200 dark:hover:bg-slate-200 dark:bg-slate-800 rounded-lg" title="编辑">✏️</button>
            <button @click="remove(n)" class="p-1.5 text-slate-600 dark:text-slate-400 hover:text-red-400 hover:bg-slate-200 dark:hover:bg-slate-200 dark:bg-slate-800 rounded-lg" title="删除">🗑️</button>
          </div>
        </div>

        <div class="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800/80 space-y-2 text-xs">
        ...
        <div class="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
      ...
      <div v-if="inbounds.length === 0" class="col-span-2 py-16 bg-white dark:bg-[#131b2e] border border-slate-200 dark:border-slate-800 rounded-2xl text-center text-slate-500 text-xs">
        当前无入站节点
      </div>
    </div>
        ...
        <form @submit.prevent="save" class="mt-4 space-y-4">
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input v-model="form.tag" required type="text" placeholder="vless-reality-8443" class="w-full px-3.5 py-2 text-sm bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl" />
            </div>
            <div>
              <label class="block text-xs font-semibold mb-1">协议类型 *</label>
              <select v-model="form.type" :disabled="editing" class="w-full px-3 py-2 text-sm bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input v-model.number="form.listen_port" required type="number" min="1" max="65535" placeholder="8443" class="w-full px-3.5 py-2 text-sm bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl" />
            </div>
            <div>
              <label class="block text-xs font-semibold mb-1">监听地址</label>
              <input v-model="form.listen" type="text" placeholder="::" class="w-full px-3.5 py-2 text-sm bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl" />
            </div>
          </div>

          <div v-if="form.type === 'vless'" class="space-y-3 p-3.5 bg-slate-100/80 dark:bg-slate-100 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl">
            ...
            <div>
              ...
              <input v-model="form.server_name" type="text" placeholder="www.amazon.com" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg" />
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-semibold mb-1">PrivateKey</label>
                <div class="flex gap-1">
                  <input v-model="form.private_key" type="text" placeholder="留空自动生成" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg font-mono" />
              ...
              <div>
                ...
                <input v-model="form.short_id" type="text" placeholder="0123456789abcdef" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg font-mono" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-semibold mb-1">Handshake Server</label>
                <input v-model="form.handshake_server" type="text" placeholder="www.amazon.com" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg" />
              </div>
              <div>
                <label class="block text-xs font-semibold mb-1">Handshake Port</label>
                <input v-model.number="form.handshake_port" type="number" placeholder="443" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg" />
              </div>
            </div>
          </div>

          <div v-else class="space-y-3 p-3.5 bg-slate-100/80 dark:bg-slate-100 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <select v-model="form.congestion_control" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg">
              ...
              <div>
                ...
                <input v-model="form.auth_timeout" type="text" placeholder="3s" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-semibold mb-1">Certificate Provider</label>
                <input v-model="form.certificate_provider" type="text" placeholder="my-acme" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg" />
              </div>
              <div>
                <label class="block text-xs font-semibold mb-1">ALPN</label>
                <input v-model="form.alpn" type="text" placeholder="h3" class="w-full px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg" />
              </div>
            </div>
            <label class="flex items-center space-x-2 text-xs">
              <input v-model="form.zero_rtt_handshake" type="checkbox" class="rounded border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950 text-cyan-500" />
