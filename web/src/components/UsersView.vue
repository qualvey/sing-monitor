<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-[#131b2e] border border-slate-800 p-4 rounded-2xl">
      <div class="relative flex-1 max-w-md">
        <input v-model="kw" type="text" placeholder="搜索用户名、Email 或 UUID..."
          class="w-full pl-4 pr-4 py-2 text-xs bg-slate-900 border border-slate-800 rounded-xl focus:outline-none focus:border-indigo-500" />
      </div>
        ...
        <tbody class="divide-y divide-slate-800/60">
          <tr v-for="u in sortedUsers" :key="u.id" class="hover:bg-slate-800/40 transition-colors">
            ...
            <td class="py-3.5 px-4">
              ...
              <div v-if="u.traffic_limit > 0" class="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden mt-1">
            ...
            <td class="py-3.5 px-4 text-slate-400">{{ fmtTime(u.expire_at) }}</td>
            <td class="py-3.5 px-4">
              <span :class="[u.enable ? (u.is_over_limit ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20') : 'bg-slate-800 text-slate-400 border-slate-700',
                'inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold border']">
                {{ u.enable ? (u.is_over_limit ? '流量超额' : '正常启用') : '已禁用' }}
              </span>
            </td>
            <td class="py-3.5 px-4 text-right space-x-2">
              <button @click="openEdit(u)" class="p-1.5 text-slate-400 hover:text-indigo-400 hover:bg-slate-800 rounded-lg" title="编辑">✏️</button>
              <button @click="remove(u)" class="p-1.5 text-slate-400 hover:text-red-400 hover:bg-slate-800 rounded-lg" title="删除">🗑️</button>
            </td>
          </tr>
        ...
        <form @submit.prevent="save" class="mt-4 space-y-4">
          <div>
            ...
            <input v-model="form.email" required type="text" placeholder="user1@example.com 或 张三"
              class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl focus:outline-none focus:border-indigo-500" />
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-semibold mb-1">UUID (VLESS / VMess)</label>
              <div class="flex gap-1">
                <input v-model="form.uuid" type="text" placeholder="自动生成" class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl font-mono" />
            ...
            <div>
              ...
              <input v-model="form.password" type="text" placeholder="pass123" class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl" />
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-semibold mb-1">流量上限 (GB, 0=不限)</label>
              <input v-model.number="form.trafficLimitGB" type="number" min="0" step="1" class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl" />
            </div>
            <div>
              <label class="block text-xs font-semibold mb-1">到期时间 (留空永久)</label>
              <input v-model="form.expireAtStr" type="datetime-local" class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl" />
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-semibold mb-1">周期天数（默认 30）</label>
              <input v-model.number="form.cycleDays" type="number" min="1" step="1" class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl" />
            </div>
            <div>
              <label class="block text-xs font-semibold mb-1">周期起始时间</label>
              <input v-model="form.cycleStartStr" type="datetime-local" class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl" />
            </div>
          </div>
          <div>
            <label class="block text-xs font-semibold mb-1.5">绑定入站节点（不勾选默认全部）</label>
            <div class="max-h-36 overflow-y-auto bg-slate-900 border border-slate-800 rounded-xl p-2 space-y-1.5">
              <div v-for="n in inbounds" :key="n.id" @click="toggleInbound(n.id)"
                class="flex items-center space-x-2.5 text-xs text-slate-200 hover:bg-slate-800/50 p-1.5 rounded-lg cursor-pointer">
                <input type="checkbox" :checked="form.inbound_ids.includes(n.id)" class="rounded border-slate-700 bg-slate-800 text-indigo-500" />
    ...
    <div v-if="cycleOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div class="bg-[#131b2e] border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl">
        <div class="flex items-center justify-between pb-4 border-b border-slate-800">
          <h3 class="text-lg font-bold">⚙️ 设置计费周期 — {{ cycleUser?.email }}</h3>
          <button @click="cycleOpen = false" class="text-slate-400 hover:text-white">✕</button>
        </div>
        <div class="mt-4 space-y-4">
          <p class="text-xs text-slate-400">周期窗口自动滚动：当前周期内流量 = 大盘显示的该用户流量。修改后立即生效。</p>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-semibold mb-1">周期天数 *</label>
              <input v-model.number="cycleForm.days" type="number" min="1" required class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl" />
            </div>
            <div>
              <label class="block text-xs font-semibold mb-1">起始时间 *</label>
              <input v-model="cycleForm.start" type="datetime-local" required class="w-full px-3.5 py-2 text-sm bg-slate-900 border border-slate-700 rounded-xl" />
            </div>
          </div>
          <div class="flex justify-end space-x-3 pt-4 border-t border-slate-800">
            <button @click="cycleOpen = false" class="px-4 py-2 text-xs text-slate-300 hover:bg-slate-800 rounded-xl">取消</button>
